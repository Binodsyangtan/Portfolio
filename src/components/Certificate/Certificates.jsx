import React, { useState, useEffect } from "react";
import { certificate } from "../../constants";

const Certificates = () => {
  const [selectedCertificate, setSelectedCertificate] = useState(null);

  const handleOpenModal = (cert) => {
    setSelectedCertificate(cert);
  };

  const handleCloseModal = () => {
    setSelectedCertificate(null);
  };

  // Prevent scroll when modal is open
  useEffect(() => {
    document.body.style.overflow = selectedCertificate ? "hidden" : "";
    return () => {
      document.body.style.overflow = "";
    };
  }, [selectedCertificate]);

  return (
    <section
      id="Certificates"
      className="py-24 px-[12vw] md:px-[7vw] lg:px-[20vw] font-sans relative"
    >
      {/* Section Title */}
      <div className="text-center mb-16">
        <h2 className="text-4xl font-bold text-white">CERTIFICATES</h2>
        <div className="w-32 h-1 bg-[#00BFFF] mx-auto mt-4"></div>
        <p className="text-gray-400 mt-4 text-lg font-semibold">
          Recognitions of my dedication to learning and professional growth
        </p>
      </div>

      {/* Certificates Grid */}
      <div className="grid gap-12 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
        {certificate.map((cert) => (
          <div
            key={cert.id}
            onClick={() => handleOpenModal(cert)}
            className="border border-[#00BFFF] bg-gray-900 backdrop-blur-md rounded-2xl shadow-2xl overflow-hidden cursor-pointer hover:shadow-[#00FFFF] hover:-translate-y-2 transition-transform duration-300"
          >
            <div className="p-4">
              <img
                src={cert.image}
                alt={cert.title}
                className="w-full h-48 object-cover rounded-xl"
              />
            </div>
            <div className="p-6">
              <h3 className="text-2xl font-bold text-white mb-2">
                {cert.title}
              </h3>
              <p className="text-gray-500 mb-4 pt-4 line-clamp-3">
                {cert.institution}
              </p>
              <span className="text-xs text-[#00BFFF] bg-gray-800 px-3 py-1 rounded-full self-start">
                {cert.date}
              </span>
            </div>
          </div>
        ))}
      </div>

      {/* Modal Container */}
      {selectedCertificate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-90 p-4">
          <div className="bg-gray-900 rounded-xl shadow-2xl lg:w-[70%] w-[70%] max-w-xl max-h-[96vh] overflow-y-auto relative py-4 border border-[#00BFFF]">
            <div className="flex justify-end p-4">
              <button
                onClick={handleCloseModal}
                className="text-white text-3xl font-bold hover:text-[#00FFFF]"
              >
                &times;
              </button>
            </div>

            <div className="flex flex-col">
              <div className="w-full flex justify-center bg-gray-900 px-4">
                <img
                  src={selectedCertificate.image}
                  alt={selectedCertificate.title}
                  className="lg:w-full w-[95%] object-contain rounded-xl shadow-2xl"
                />
              </div>
              <div className="lg:p-8 p-6">
                <h3 className="lg:text-3xl font-bold text-white mb-4 text-md">
                  {selectedCertificate.title}
                </h3>
                <p className="text-gray-400 mb-4 lg:text-base text-xs">
                  <strong>Institution:</strong> {selectedCertificate.institution}
                </p>
                <p className="text-gray-400 mb-4 lg:text-base text-xs">
                  <strong>Date:</strong> {selectedCertificate.date}
                </p>

                {selectedCertificate.credentialId && (
                  <p className="text-gray-400 mb-4 lg:text-base text-xs">
                    <strong>Credential ID:</strong> {selectedCertificate.credentialId}
                  </p>
                )}

                {selectedCertificate.description && (
                  <p className="text-gray-400 mb-6 lg:text-base text-xs">
                    <strong>Skills:</strong> {selectedCertificate.description}
                  </p>
                )}

                {selectedCertificate.tags && (
                  <div className="flex flex-wrap gap-2 mb-6">
                    {selectedCertificate.tags.map((tag, index) => (
                      <span
                        key={index}
                        className="bg-[#251f38] text-xs font-semibold text-[#00BFFF] rounded-full px-2 py-1"
                      >
                        {tag}
                      </span>
                    ))}
                  </div>
                )}
              </div>
            </div>
          </div>
        </div>
      )}
    </section>
  );
};

export default Certificates;
